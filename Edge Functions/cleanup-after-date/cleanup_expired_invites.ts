...
const CF_BASE_URL =
  "https://api.cloudflare.com/client/v4/accounts/dummy_account";
...
serve(async (req: Request) => {
  try {
    ...
    if (!expiredInvites || expiredInvites.length === 0) {
      return new Response(
        JSON.stringify({ message: "No expired invites found" }),
        {
          headers: { "Content-Type": "application/json" },
        }
      );
      ...
      if (record.cloudflareID) {
        try {
          const response = await fetch(
            `${CF_BASE_URL}/stream/${record.cloudflareID}`,
            {
              method: "DELETE",
              headers: {
          ...
          if (!response.ok) {
            console.error(
              `Failed to delete Cloudflare video ${record.cloudflareID}:`,
              await response.text()
            );
          }
        } catch (error) {
          console.error(
            `Error deleting Cloudflare video ${record.cloudflareID}:`,
            error
          );
        }
      }
    ...
    if (deleteRemoteError) {
      throw new Error(
        `Error deleting remote records: ${deleteRemoteError.message}`
      );
    }
    ...
    if (deleteInviteError) {
      throw new Error(
        `Error deleting invite records: ${deleteInviteError.message}`
      );
    }
